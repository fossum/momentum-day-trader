...
const replacement2 = `            {/* Blacklisted Tickers */}
            <div className="border-t border-zinc-800 pt-3 mb-3">
              <span className="block text-xs font-bold text-zinc-300 mb-2">Blacklisted Tickers (Do Not Trade):</span>
              <input
                type="text"
                value={(preferences.blacklistedTickers || []).join(', ')}
                onChange={(e) => {
                  const val = e.target.value;
                  const newPrefs = {
                    ...preferences,
                    blacklistedTickers: val.split(',').map(s => s.trim().toUpperCase()).filter(s => s)
                  };
                  setPreferences(newPrefs);
                }}
                onBlur={() => savePreferences(preferences)}
...
if(code.includes(target1) && code.includes(target2)){
  ...
  console.log("Replaced patch3");
} else {
  console.log("Not found patch3");
}
