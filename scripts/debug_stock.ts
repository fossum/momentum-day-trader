/**
 * @fileoverview Utility script to debug individual stock setups.
 * Fetches 1-minute historical chart data, shares float, profile details,
 * and runs the bull flag pattern analysis for a single ticker to diagnose strategy decisions.
 */
...
/**
 * Main execution function to debug a single stock's momentum strategy setup.
 * Fetches chart data and company details, then prints bull flag pattern detection analysis.
 * 
 * @returns A promise that resolves when the debugging execution completes.
 */
async function main() {
  ...
  if (!Array.isArray(chart) || chart.length === 0) {
    console.error(`Error: No 1min chart data returned for ${symbol}. The response might be empty or an error payload.`);
    process.exit(1);
  }
